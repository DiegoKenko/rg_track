...
class FlespiServiceMultiple {
  FlespiServiceMultiple();
  Dio dio = getIt<Dio>();

  Future<Result<List, ErrorEntity>> get(
    String url, {
  ...
  Future<Result<List, ErrorEntity>> post(
    String url, {
    dynamic data,
    Map<String, dynamic>? params,
    Map<String, dynamic>? headers,
  ...
  Future<Result<List, ErrorEntity>> put(
    String url, {
    Map<String, dynamic>? params,
    Map<String, dynamic>? headers,
  }) async {
  ...
  Future<Result<List, ErrorEntity>> delete(
    String url, {
    Map<String, dynamic>? params,
    Map<String, dynamic>? headers,
  }) async {
  ...
  Result<List<dynamic>, ErrorEntity> _convertResponse(
    Response response,
  ) {
    try {
      if (Map.from(response.data).isNotEmpty) {
        if (Map.from(response.data).keys.contains('result')) {
          if (Map.from(response.data)['result'] is List) {
            return (Map.from(response.data)['result'] as List).toSuccess();
          }
        }
      }
